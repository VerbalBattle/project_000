...
.controller('LobbyCtrl', function ($scope, $location, GamePlay, Match, socketFactory, toaster, $rootScope) {
  ...
  if (!$rootScope.socketLobby) {
    ...
    mySocket.on('client:turnUpdate', function (data) {
      var opponentName = data.rooms[Object.keys(data.rooms)[0]]
          .avatar1.avatarName;
      // Get the avatarID
      ...
      if (!user.avatars[avatarID]) {
        ...
        avatarID = data.rooms[Object.keys(data.rooms)[0]].avatar2.avatarID;
        opponentName = data.rooms[Object.keys(data.rooms)[0]].avatar2.avatarName;
      }
