...
Ranking.getNewRank = function (player1, player2, winner) {
  var player1Stats = player1.avatarStats;
  var player2Stats = player2.avatarStats;

  // build the winTuple based on who won the game
  var winTuple = (winner === player1.avatarID) ? [1,0] : [0,1];

  // calculate the expected percentage that personX will beat personY based on both of their rankings
  var expectedA = 1.0 / (1.0 + Math.pow((player2Stats.elo - player1Stats.elo) / 400.0, 10));
  var expectedB = 1.0 / (1.0 + Math.pow((player1Stats.elo - player2Stats.elo) / 400.0, 10));

  // rounding up so the winning player always gains some points,
  // ensure the rating never drops below zero.
  var player1Rating = Math.max(Math.ceiling(player1Stats.elo + 32.0 * (winTuple[0] - expectedA)), 0);
  var player2Rating = Math.max(Math.ceiling(player2Stats.elo + 32.0 * (winTuple[1] - expectedB)), 0);
