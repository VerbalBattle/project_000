...
var matchBatch = function (n) {
  ...
  while (curPlayer) {
    ...
    if (i > n) {
      matches = matches.concat(matchGroup(players, waitingForGame.invalidMatches));
      i = 0;
      batch = {};
    }
  ...
  if (batch !== {}) {
    matches = matches.concat(matchGroup(batch, waitingForGame.invalidMatches));
  }
