...
var matchBatch = function (n, badMatches, naiveMatch, players) {
  ...
  while (curPlayer) {
    ...
    if (i > n) {
      var temp = cloneObj(batch);
      // var results = matchGroup(batch, badMatches, naiveMatch);
      var results = matchGroup(players, waitingForGame.invalidMatches, naiveMatch);
      matches = matches.concat(results[0]);
      var badMatches = results[1];
  ...
  if (Object.keys(batch).length > 0) {
    var temp = cloneObj(batch);
    // var results = matchGroup(batch, badMatches, naiveMatch);
    var results = matchGroup(batch, waitingForGame.invalidMatches, naiveMatch);
    matches = matches.concat(results[0]);
    var badMatches = results[1];
  ...
  if (batchRejects.head) {
   ...
   var results = matchBatch(n, waitingForGame.invalidMatches, false, batchRejects);
  ...
  if (naiveBatch.head) {
    ...
    var results = matchBatch(n, waitingForGame.invalidMatches, true, naiveBatch);
