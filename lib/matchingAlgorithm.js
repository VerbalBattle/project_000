...
var findOpponent = function ( playerObj, otherPlayers, features, badMatches, naiveMatch) {
  ...
  otherPlayersKeys.forEach( function ( name) {
    ...
    if ( naiveMatch || (Math.abs(Ranking.determineRank(stats.elo) - playerRank) <= rankMargin && (!badMatches[playerObj.userID] || !badMatches[playerObj.userID][player.userID]))) {
...
var matchBatch = function (n, badMatches, naiveMatch, players) {
  ...
  if (batchRejects.head && batchRejects.next) {
