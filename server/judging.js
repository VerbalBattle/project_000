...
// Judging function to update rooms' timers
...
judging.archiveRoom = function (roomID) {
  // Get reference to this instance
  var that = this;
  ...
  roomsTable.find({
    ...
    if (roomFound) {
      ...
      // Set voting data for clients
      var keyStr = '';
      for (var data in [pData1, pData2]) {
        data[pData1.avatarID] = avatar1Votes;
        data[pData2.avatarID] = avatar2Votes;
      }
      ...
      avatarStatsTable.findAll({
        ...
        for (var i = 0; i < avatarStatsFound.length; ++i) {
          ...
          // Add stats to data to send to client
          if (avatarStats.dataValues.id === pData1.avatarID) {
            pData1.avatarStats = avatarStats.dataValues;
          } else if (avatarStats.dataValues.id === pData2.avatarID) {
            pData2.avatarStats = avatarStats.dataValues;
          }
...
// Initialize for judging
judging.initialize = function (cb) {
  // Set callback to use when archiving
  this.socketJudgementCallback = cb;
  // This binding
  var that = this;
